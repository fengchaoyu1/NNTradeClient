...
public class TestThumbnails extends InstrumentationTestCase{
    ...
    public void Creatpicture() {
        ...
        final int STRIDE = 500;//must be >=WIDTH
    ...
    public void testThumbnailsBig() throws Exception {
        ...
        bitmapBig = getImageThumbnail(ImagePath,600,700);
        File file = new File(Environment.getExternalStorageDirectory() +"/", "ooo" + ".jpg");
        FileOutputStream output = new FileOutputStream(file);
        try {
            bitmapBig.compress(Bitmap.CompressFormat.JPEG, 30, output);
            output.flush();
            output.close();
        }  catch (FileNotFoundException e) {
            e.printStackTrace();
    ...
    public void testThumbnailsSmall() throws Exception {
        Creatpicture();
        String ImagePath = Environment.getExternalStorageDirectory() + "/"+"abc" + ".jpg";
        bitmapSmall = getImageThumbnail(ImagePath, 150, 200);
        File file = new File(Environment.getExternalStorageDirectory() + "/","ooo" + ".jpg");
        FileOutputStream output = new FileOutputStream(file);
        try {
            bitmapSmall.compress(Bitmap.CompressFormat.JPEG, 30, output);
            output.flush();
            output.close();
        }  catch (FileNotFoundException e) {
            e.printStackTrace();
